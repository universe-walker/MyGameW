...
export class GameGateway implements OnGatewayInit, OnGatewayConnection {
  ...
  afterInit(server: Server) {
    ...
    server.of('/game').use((socket, next) => {
      const initDataRaw = socket.handshake.auth?.initDataRaw as string | undefined;
      const token = process.env.TELEGRAM_BOT_TOKEN || process.env.BOT_TELEGRAM_BOT_TOKEN || '';
      if (!initDataRaw || !verifyInitData(initDataRaw, token).ok) return next(new Error('unauthorized'));
      next();
    });
  }
