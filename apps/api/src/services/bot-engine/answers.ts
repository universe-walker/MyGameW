...
type BotProfile = ReturnType<BotEngineService['profiles']['getAll']>[number];
type ActiveBot = { playerId: number; profile: BotProfile };
...
export function scheduleBotBuzz(engine: BotEngineService, roomId: string): void {
  ...
  for (const { playerId, profile } of bots) {



    const [min, max] = profile.buzzReactionMs;
    ...
    const delay = Math.max(0, Math.floor(min + engine.rand() * span));



    engine.timers.set(roomId, `bot_buzz_${playerId}`, delay, () => {
      ...
      if (!current || current.phase !== 'buzzer_window' || current.activePlayerId != null) return;
      ...
      const buzzChance = answerProbability(knowledge, value, profile.riskProfile);



      const blind = engine.rand() < profile.blindBuzzRate;



      if (!blind && engine.rand() >= buzzChance) return;
      ...
      if (Array.isArray(current.order)) {



        const idx = current.order.indexOf(playerId);



        if (idx >= 0) current.answerIndex = idx;
      ...
      engine.scheduleBotThinkAndAnswer(roomId, playerId);
...
export function pickBotsForRoom(engine: BotEngineService, roomId: string): ActiveBot[] {
  ...
  if (!runtime?.botProfiles || runtime.botProfiles.size === 0) return [];



  return Array.from(runtime.botProfiles.entries()).map(([playerId, profile]) => ({ playerId, profile }));
